...
const Footer: React.FC<Props> = ({ onOpenDeveloper, onOpenProfile, onNavigate, isDarkMode, variant = 'default' }) => {
  ...
  return (
    <footer className={`border-t transition-colors duration-500 ${isDarkMode ? 'bg-[#02040a] border-white/5 text-slate-400' : 'bg-white/40 border-slate-100 text-slate-500'}`}>
              ...
              <div className="flex flex-wrap gap-3">
                  {socialLinks.map((social, i) => (
                      <a 
                          ...
                          className="p-3 rounded-full bg-white/10 hover:bg-white/10 hover:text-white transition-all border border-transparent hover:border-white/10"
                          title={social.label}
                      >
                          <social.icon size={18} />
                      </a>
                  ...
                  <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="p-3 rounded-full bg-white/10 hover:bg-white/10 hover:text-white transition-all opacity-60 hover:opacity-100"
                      title="Moodle (Legacy)"
                  >
                      <GraduationCap size={18} />
                  </a>
            ...
            <div className="flex items-center gap-4">
                ...
                <button 
                    ...
                    className={`flex items-center gap-2 px-3 py-1 rounded-full border transition-all ${
                        isDarkMode 
                        ? 'border-white/10 bg-white/10 hover:bg-white/10 text-slate-400 hover:text-white' 
                ...
                <div className="flex items-center gap-2 text-xs font-bold">
                    ...
                    <button onClick={onOpenProfile} className={`flex items-center gap-2 px-3 py-1 rounded-full ${isDarkMode ? 'bg-white/10 hover:bg-white/10 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-900'} transition-all`}>
