...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  if (activeContext === 'Global') {
    if (isSuperAdmin) {
      menuItems.push(
        ...
        { id: 'faculty-registry', label: 'Faculty Hub', icon: UserCog }, // Added Faculty Hub
      ...
      menuItems.push(
        ...
        { id: 'recruitment', label: 'Recruitment Status', icon: Briefcase }, // Added Recruitment
        { id: 'my-certificates', label: 'Asset Ledger', icon: Award },
        { id: 'tickets', label: 'Session Tickets', icon: Ticket },
        { id: 'payments', label: 'Financial Ledger', icon: CreditCard } // Added Payments
      );
    }
  } else {
    menuItems.push(
  ...
  return (
    <aside className={`
      fixed inset-y-0 left-0 z-50 w-80 flex flex-col p-6
      transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} 
      md:translate-x-0 md:relative md:h-screen
      transition-all duration-700 ease-in-out
    `}>
      <div className={`w-full h-full rounded-[3rem] flex flex-col overflow-hidden border shadow-2xl transition-all ${isDarkMode ? 'bg-[#0d121d]/40 backdrop-blur-3xl border-white/5 shadow-black/40' : 'bg-white/60 backdrop-blur-3xl border-white shadow-blue-500/5'
        }`}>
        ...
        <div className="p-8 pb-4">
          <div className="flex items-center justify-between mb-8">
            ...
            <button onClick={onClose} className="md:hidden p-2 hover:bg-white/5 rounded-xl">
          ...
          <div className="relative" ref={switcherRef}>
            ...
            {isSwitcherOpen && (
              <div className={`absolute left-0 right-0 top-full mt-3 z-[100] border rounded-[2rem] shadow-[0_32px_64px_rgba(0,0,0,0.3)] overflow-hidden p-2 animate-in fade-in slide-in-from-top-4 duration-300 backdrop-blur-3xl ${isDarkMode ? 'bg-[#1a202e]/90 border-white/10' : 'bg-white/90 border-slate-100'
                }`}>
                <div className="space-y-1 max-h-[250px] overflow-y-auto custom-scrollbar">
                  <button
                    onClick={() => handlePerspectiveSelect('Global')}
                    className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all ${isDarkMode ? 'hover:bg-white/5 text-white' : 'hover:bg-blue-500/5 text-[#1B2559]'}`}
                  >
