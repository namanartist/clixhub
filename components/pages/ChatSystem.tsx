...
const ChatSystem: React.FC<Props> = ({ user, clubs, allUsers, activeContext, isDarkMode }) => {
    ...
    return (
        <div className={`flex h-[calc(100vh-6rem)] md:h-[calc(100vh-4rem)] overflow-hidden bg-[#050505] text-[var(--text-main)] font-sans`}>
                ...
                <div className="p-6 md:p-8 space-y-6 md:space-y-8">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-4">
                            <button className="p-3 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all"><Hexagon size={20}/></button>
                            <button className="p-3 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all"><Cpu size={20}/></button>
                        </div>
                    </div>
                    <div className="relative">
                        <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-primary" size={20} />
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Ledger..." className="w-full h-16 bg-white/5 border border-white/10 rounded-2xl pl-16 pr-8 font-black text-xs uppercase tracking-widest outline-none focus:border-primary/50" />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-4 space-y-2 pb-10">
                    ...
                    {filteredUsers.map(u => (
                        <div key={u.id} onClick={() => setActiveChannel({ type: 'dm', id: u.id, name: u.name, image: u.photoUrl, subtitle: u.globalRole, isOnline: u.isOnline, lastSeen: u.lastSeen })}
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center">
                                    <h3 className="font-black text-lg tracking-tight truncate">{u.name}</h3>
            ...
            {/* ─ CHAT VIEW ─ */}
            <div className={`flex-1 flex flex-col bg-[#050505] relative transition-all duration-500 ${!activeChannel ? 'hidden md:flex' : 'flex'}`}>
                {activeChannel ? (
                    <>
                        {/* Header */}
                        <div className="h-24 px-5 md:px-8 border-b border-white/5 flex items-center justify-between bg-white/2 backdrop-blur-3xl z-40">
                            ...
                            <div className="hidden lg:flex gap-4">
                               <button className="p-4 bg-white/5 border border-white/10 rounded-2xl text-white hover:bg-white/10 transition-all"><Video size={20}/></button>
                               <button className="p-4 bg-white/5 border border-white/10 rounded-2xl text-white hover:bg-white/10 transition-all"><Phone size={20}/></button>
                               <button className="p-4 bg-white/5 border border-white/10 rounded-2xl text-white hover:bg-white/10 transition-all"><MoreVertical size={20}/></button>
                            </div>
                        </div>
                           ...
                           {messages.map((msg, i) => {
                               ...
                               return (
                                   <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} reveal`}>
                                       {showHeader && activeChannel.type === 'club' && !isMe && <span className="text-[10px] font-black uppercase tracking-widest text-primary ml-4 mb-2">{msg.senderName}</span>}
                                       <div className={`relative max-w-[70%] lg:max-w-[50%] p-6 rounded-[2rem] shadow-3xl group ${isMe ? 'bg-primary text-white rounded-tr-none' : 'bg-white/5 border border-white/10 text-white rounded-tl-none'}`}>
                                              ...
                                              <div className="space-y-4">
                                                 ...
                                                 <div className="space-y-2">
                                                    {msg.pollOptions?.map(opt => (
                                                       <div key={opt.id} className="relative h-12 bg-white/5 border border-white/5 rounded-xl overflow-hidden flex items-center px-4 cursor-pointer hover:bg-white/10 transition-all">
                                           ...
                                           <div className={`mt-4 flex items-center gap-2 text-[8px] font-black opacity-40 uppercase tracking-widest ${isMe ? 'text-white' : 'text-primary'}`}>
                        ...
                        <div className="p-5 md:p-8 bg-white/2 border-t border-white/5 relative z-50">
                           ...
                           <div className="max-w-7xl mx-auto flex items-center gap-6">
                              <button onClick={() => setShowAttachMenu(!showAttachMenu)} className={`h-16 w-16 rounded-2xl flex items-center justify-center transition-all ${showAttachMenu ? 'bg-primary text-white rotate-45' : 'bg-white/5 border border-white/10 text-white'}`}><Plus size={32}/></button>
                              <form onSubmit={e => { e.preventDefault(); handleSend('text'); }} className="flex-1 h-16 bg-white/5 border border-white/10 rounded-3xl flex items-center px-8 gap-6 group focus-within:border-primary/50 transition-all">
                                 <input value={input} onChange={e => setInput(e.target.value)} placeholder="Establish Uplink Signal..." className="flex-1 bg-transparent border-none outline-none font-black text-xs uppercase tracking-[0.2em] text-white placeholder-white/10" />
                                 <button type="submit" className={`h-12 w-12 rounded-xl flex items-center justify-center transition-all ${input.trim() ? 'bg-primary text-white shadow-xl shadow-primary/30' : 'text-white/20'}`}><Send size={24}/></button>
                              </form>
                           </div>
                        </div>
                        ...
                        {showPollModal && (
                           <div className="fixed inset-0 z-[1000] bg-black/98 flex items-center justify-center p-8">
                              <div className="relative w-full max-w-md bento-card p-12 space-y-10 animate-in zoom-in-95 duration-300">
                                 <div className="flex justify-between items-center">
                                    <h3 className="text-3xl font-[1000] tracking-tighter uppercase italic">Index Poll</h3>
                                    <button onClick={() => setShowPollModal(false)} className="p-3 bg-white/5 rounded-xl text-white"><X size={24}/></button>
                                 </div>
                                 <input placeholder="Index Question?" className="w-full h-18 bg-white/5 border border-white/10 rounded-2xl px-6 font-black text-lg text-white" />
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-20 space-y-10 opacity-30">
                        <div className="p-10 bg-white/5 border border-dashed border-white/10 rounded-full animate-pulse-slow"><Globe2 size={120}/></div>
                        <div className="space-y-4">
