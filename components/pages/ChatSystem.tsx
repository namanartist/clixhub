...
const ChatSystem: React.FC<Props> = ({ user, clubs, allUsers, activeContext, isDarkMode }) => {
    ...
    return (
        <div className={`flex h-[calc(100vh-6rem)] md:h-[calc(100vh-4rem)] overflow-hidden ${isDarkMode ? 'bg-[#050505] text-white' : 'bg-[#f8f9fa] text-slate-900'} font-sans`}>
            ...
            {/* ─ SIDEBAR ─ */}
            <div className={`w-full md:w-[450px] flex flex-col border-r absolute md:relative inset-0 z-50 transition-all duration-500 ${isDarkMode ? 'border-white/5 bg-white/' : 'border-slate-200 bg-white/'} ${activeChannel ? '-translate-x-full md:translate-x-0' : 'translate-x-0'}`}>
                <div className="p-6 md:p-8 space-y-6 md:space-y-8">
                    <div className="flex justify-between items-center">
                        <h1 className="text-4xl font-[1000] tracking-tighter uppercase italic leading-none">Channels</h1>
                        <div className="flex gap-4">
                            <button className={`p-3 rounded-2xl border transition-all ${isDarkMode ? 'bg-white/ border-white/5 hover:bg-white/' : 'bg-whiteborder-slate-200 hover:bg-slate-50shadow-sm'}`}><Hexagon size={20}/></button>
                            <button className={`p-3 rounded-2xl border transition-all ${isDarkMode ? 'bg-white/ border-white/5 hover:bg-white/' : 'bg-whiteborder-slate-200 hover:bg-slate-50shadow-sm'}`}><Cpu size={20}/></button>
                        </div>
                    </div>
                    <div className="relative">
                        <Search className={`absolute left-6 top-1/2 -translate-y-1/2 ${isDarkMode ? 'text-primary' : 'text-slate-400'}`} size={20} />
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Ledger..." className={`w-full h-16 rounded-2xl pl-16 pr-8 font-black text-xs uppercase tracking-widest outline-none focus:border-primary/50 transition-all ${isDarkMode ? 'bg-white/ border border-white/10' : 'bg-whiteborder border-slate-200 shadow-sm'}`} />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar px-4 space-y-2 pb-10">
                    ...
                    {filteredClubs.map(club => (
                        <div key={club.id} onClick={() => setActiveChannel({ type: 'club', id: club.id, name: club.name, color: club.themeColor, subtitle: `${club.category} Council` })}
                             className={`p-5 md:p-6 rounded-[2rem] border transition-all cursor-pointer flex items-center gap-5 group 
                                ${activeChannel?.id === club.id ? 'bg-primary/10 border-primary/40' : 'bg-white/ border-white/5 hover:bg-white/'}`}>
                    ...
                    {filteredUsers.map(u => (
                        <div key={u.id} onClick={() => setActiveChannel({ type: 'dm', id: u.id, name: u.name, image: u.photoUrl, subtitle: u.globalRole, isOnline: u.isOnline, lastSeen: u.lastSeen })}
                             className={`p-5 md:p-6 rounded-[2rem] border transition-all cursor-pointer flex items-center gap-5 group 
                                ${activeChannel?.id === u.id ? 'bg-primary/10 border-primary/40' : (isDarkMode ? 'bg-white/ border-white/5 hover:bg-white/' : 'bg-whiteborder-slate-200 hover:border-primary/30 shadow-sm')}`}>
                            <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center relative overflow-hidden ${isDarkMode ? 'bg-white/ border-white/5' : 'bg-slate-50border-slate-100'}`}>
            ...
            <div className={`flex-1 flex flex-col relative transition-all duration-500 ${isDarkMode ? 'bg-[#050505]' : 'bg-[#f8f9fa]'} ${!activeChannel ? 'hidden md:flex' : 'flex'}`}>
                {activeChannel ? (
                    <>
                        {/* Header */}
                        <div className={`h-24 px-5 md:px-8 border-b flex items-center justify-between z-40 ${isDarkMode ? 'border-white/5 bg-white/ backdrop-blur-3xl' : 'border-slate-200 bg-white/ backdrop-blur-xl'}`}>
                            <div className="flex items-center gap-6">
                                <button onClick={() => setActiveChannel(null)} className="md:hidden p-3 bg-white/ border border-white/10 rounded-2xl text-white"><ArrowLeft size={20}/></button>
                                <div className="flex items-center gap-5">
                                    {activeChannel.type === 'club' ? (
                                        ...
                                        <div className={`w-14 h-14 rounded-2xl border overflow-hidden flex items-center justify-center ${isDarkMode ? 'bg-white/ border-white/10' : 'bg-slate-50border-slate-200'}`}>
                            ...
                            <div className="hidden lg:flex gap-4">
                               <button className={`p-4 rounded-2xl transition-all ${isDarkMode ? 'bg-white/ border border-white/10 text-white hover:bg-white/' : 'bg-whiteborder border-slate-200 text-slate-500 hover:bg-slate-50shadow-sm'}`}><Video size={20}/></button>
                               <button className={`p-4 rounded-2xl transition-all ${isDarkMode ? 'bg-white/ border border-white/10 text-white hover:bg-white/' : 'bg-whiteborder border-slate-200 text-slate-500 hover:bg-slate-50shadow-sm'}`}><Phone size={20}/></button>
                               <button className={`p-4 rounded-2xl transition-all ${isDarkMode ? 'bg-white/ border border-white/10 text-white hover:bg-white/' : 'bg-whiteborder border-slate-200 text-slate-500 hover:bg-slate-50shadow-sm'}`}><MoreVertical size={20}/></button>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-6 custom-scrollbar bg-dots" ref={scrollRef}>
                           <div className="flex justify-center mb-10">
                              <div className={`px-6 py-2 border rounded-full text-[10px] font-black uppercase tracking-[0.4em] opacity-50 flex items-center gap-3 ${isDarkMode ? 'bg-white/ border-white/5' : 'bg-slate-200 border-slate-300 text-slate-700'}`}>
                           ...
                           {messages.map((msg, i) => {
                               ...
                               return (
                                   <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} reveal`}>
                                       ...
                                       <div className={`relative max-w-[70%] lg:max-w-[50%] p-6 rounded-[2rem] shadow-3xl group ${isMe ? 'bg-primary text-white rounded-tr-none' : (isDarkMode ? 'bg-white/ border border-white/10 text-white rounded-tl-none' : 'bg-whiteborder border-slate-200 text-slate-900 rounded-tl-none')}`}>
                                              ...
                                              <div className="space-y-4">
                                                 ...
                                                 <div className="space-y-2">
                                                    {msg.pollOptions?.map(opt => (
                                                       <div key={opt.id} className={`relative h-12 border rounded-xl overflow-hidden flex items-center px-4 cursor-pointer transition-all ${isDarkMode ? 'bg-white/ border-white/5 hover:bg-white/' : 'bg-slate-50border-slate-200 hover:bg-slate-100'}`}>
                        ...
                        {/* Input Area */}
                        <div className={`p-5 md:p-8 border-t relative z-50 ${isDarkMode ? 'bg-white/ border-white/5' : 'bg-white/ border-slate-200 backdrop-blur-xl'}`}>
                           {showAttachMenu && (
                           ...
                           <div className="max-w-7xl mx-auto flex items-center gap-6">
                              <button onClick={() => setShowAttachMenu(!showAttachMenu)} className={`h-16 w-16 rounded-2xl flex items-center justify-center transition-all ${showAttachMenu ? 'bg-primary text-white rotate-45' : (isDarkMode ? 'bg-white/ border border-white/10 text-white' : 'bg-whiteborder border-slate-200 text-slate-500 shadow-sm')}`}><Plus size={32}/></button>
                              <form onSubmit={e => { e.preventDefault(); handleSend('text'); }} className={`flex-1 h-16 rounded-3xl flex items-center px-8 gap-6 group transition-all ${isDarkMode ? 'bg-white/ border border-white/10 focus-within:border-primary/50' : 'bg-whiteborder border-slate-200 shadow-sm focus-within:border-primary'}`}>
                           ...
                           <div className="fixed inset-0 z-[1000] bg-black/98 flex items-center justify-center p-8">
                              <div className={`relative w-full max-w-md bento-card p-12 space-y-10 animate-in zoom-in-95 duration-300 ${!isDarkMode && 'bg-white}`}>
                                 <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={() => setShowPollModal(false)} className={`p-3 rounded-xl ${isDarkMode ? 'bg-white/ text-white' : 'bg-slate-100 text-slate-500'}`}><X size={24}/></button>
                                 </div>
                                 <input placeholder="Index Question?" className={`w-full h-18 border rounded-2xl px-6 font-black text-lg ${isDarkMode ? 'bg-white/ border-white/10 text-white' : 'bg-slate-50border-slate-200 text-slate-900'}`} />
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-20 space-y-10 opacity-30">
                        <div className={`p-10 border border-dashed rounded-full animate-pulse-slow ${isDarkMode ? 'bg-white/ border-white/10 text-white' : 'bg-slate-200 border-slate-400 text-slate-800'}`}><Globe2 size={120}/></div>
                        <div className="space-y-4">
