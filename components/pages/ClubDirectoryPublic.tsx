...
const ClubDirectoryPublic: React.FC<Props> = ({ clubs, onBack, isDarkMode = true }) => {
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-[#0099FF] selection:text-white ${isDarkMode ? 'bg-[#02040a] text-white' : 'bg-[#F4F7FE] text-[#2B3674]'}`}>
      ...
      {/* Filters & Search */}
      <section className={`py-8 px-6 border-b sticky top-0 z-40 backdrop-blur-xl ${isDarkMode ? 'bg-[#02040a]/80 border-white/[0.06]' : 'bg-white/80 border-slate-200'}`}>
        <div className="max-w-[1200px] mx-auto">
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            ...
            <div className={`flex gap-2 p-2 rounded-2xl border overflow-x-auto ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-slate-200'}`}>
              {categories.map(cat => (
                <button
                  key={cat}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredClubs.map(club => (
                <div
                  ...
                  className={`group relative p-8 rounded-3xl border transition-all hover:scale-[1.02] hover:-translate-y-1 cursor-pointer overflow-hidden ${isDarkMode ? 'bg-[#0d121d] border-white/10 hover:border-cyan-400/50 hover:shadow-2xl' : 'bg-white border-slate-200 hover:shadow-xl'}`}
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <div className={`pt-6 border-t flex items-center justify-between ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                      ...
                      <button
                        ...
                        className={`p-3 rounded-full transition-all active:scale-95 ${isDarkMode ? 'bg-white/5 text-cyan-400 hover:bg-cyan-500 hover:text-white' : 'bg-slate-100 text-cyan-600 hover:bg-cyan-500 hover:text-white'}`}
                      >
