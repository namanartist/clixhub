      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-10">
         ...
         <div className="flex gap-4 w-full md:w-auto">
            <button 
              ...
              className="flex-1 md:flex-none glass-elevated border border-white/5 text-white px-8 py-5 rounded-[1.8rem] font-black text-[10px] uppercase tracking-[0.2em] shadow-xl flex items-center justify-center gap-4 hover:bg-white/10 transition-all active:scale-95"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
        {[
          ...
          <div key={i} className="glass-elevated p-10 rounded-[3.5rem] border transition-all duration-700 hover:scale-[1.02] flex flex-col justify-between h-64 group shadow-xl relative overflow-hidden">
             <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity" />
             ...
             <div className="space-y-6">
                <div className="space-y-2">
                    ...
                    <select 
                        ...
                        className="w-full bg-white/10 border border-white/5 rounded-3xl px-6 py-4 text-white font-black text-sm outline-none focus:border-blue-500/40 transition-all appearance-none uppercase tracking-widest"
                    >
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full bg-white/10 border border-white/5 rounded-3xl px-6 py-4 text-blue-400 font-mono text-xs outline-none focus:border-blue-500/40 shadow-inner"
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <input 
                                    ...
                                    placeholder="••••••••••••••••"
                                    className="w-full bg-white/10 border border-white/5 rounded-3xl px-6 py-4 text-white font-mono text-xs outline-none focus:border-blue-500/40 shadow-inner"
                                />
                            </div>
                        </div>
                    </div>
             ...
             <div className="space-y-6 max-h-[450px] overflow-y-auto pr-4 custom-scrollbar">
                {(club.quotations || []).length === 0 ? (
                  ...
                  (club.quotations || []).map(quote => (
                    <div key={quote.id} className="p-8 rounded-[2.5rem] bg-white/10[0.03] border border-white/5 space-y-6 group hover:bg-white/10[0.06] transition-all hover:border-white/10 relative overflow-hidden">
                       <div className="flex justify-between items-start relative z-10">
                          <div className="space-y-1">
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-500">
          ...
          <div className="relative max-w-2xl w-full p-12 rounded-[4rem] border border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.5)] space-y-10 animate-in zoom-in-95 duration-500 glass-elevated">
            <div className="flex justify-between items-start">
              ...
              <button onClick={() => setIsQuoteModalOpen(false)} className="p-4 rounded-3xl bg-white/10 text-slate-500 hover:text-white hover:bg-rose-500 transition-all group">
            ...
            <form onSubmit={handleAddQuotation} className="space-y-8">
               <div className="space-y-3">
                  ...
                  <input required value={newQuote.title} onChange={e => setNewQuote({...newQuote, title: e.target.value})} placeholder="E.G. TITAN MISSION LOGISTICS" className="w-full bg-white/10 border border-white/5 px-8 py-5 rounded-[2rem] outline-none focus:border-blue-500/50 text-white font-black uppercase tracking-tighter text-lg shadow-inner" />
               </div>
               <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <input required value={newQuote.vendor} onChange={e => setNewQuote({...newQuote, vendor: e.target.value})} placeholder="NODE-XX" className="w-full bg-white/10 border border-white/5 px-8 py-5 rounded-[2rem] outline-none focus:border-blue-500/50 text-white font-black uppercase tracking-widest text-sm shadow-inner" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-600 ml-8 italic">Fiscal Sum (₹)</label>
                    <input required type="number" value={newQuote.amount} onChange={e => setNewQuote({...newQuote, amount: Number(e.target.value)})} placeholder="0.00" className="w-full bg-white/10 border border-white/5 px-8 py-5 rounded-[2rem] outline-none focus:border-blue-500/50 text-white font-black text-xl shadow-inner" />
                  </div>
               </div>
               <div className="space-y-3">
                  ...
                  <textarea required value={newQuote.desc} onChange={e => setNewQuote({...newQuote, desc: e.target.value})} rows={3} placeholder="Strategic justification for credit allocation..." className="w-full bg-white/10 border border-white/5 px-8 py-6 rounded-[2.5rem] outline-none focus:border-blue-500/50 text-white font-medium text-sm leading-relaxed shadow-inner" />
               </div>
