                ...
                <div key={member.id} className="p-8 rounded-[3.5rem] flex flex-col items-center text-center relative group transition-all duration-700 hover:scale-[1.02] glass-elevated border shadow-3xl hover:shadow-blue-500/10">
                    ...
                    {isAdmin && (
                      <div className="absolute top-6 right-8 flex gap-2">
                        <button onClick={() => { setEditingMember(member); setIsEditModalOpen(true); }} className="p-2 rounded-xl bg-white/10 text-slate-400 hover:text-blue-500 hover:bg-white/10 transition-all">
                           <Edit3 size={16} />
                        </button>
                      </div>
                    )}
                    ...
                    <button 
                        ...
                        className="w-full py-4 rounded-[1.5rem] text-[9px] font-black uppercase tracking-[0.2em] transition-all bg-white/10 text-slate-400 hover:bg-blue-600 hover:text-white hover:shadow-xl hover:shadow-blue-500/20"
                    >
                        Access Dossier
                    </button>
                </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 backdrop-blur-3xl bg-black/80 animate-in fade-in duration-500">
           <div className="relative max-w-xl w-full rounded-[4rem] border border-white/10 glass-elevated p-12 space-y-8 animate-in zoom-in-95 duration-500">
              <button onClick={() => setIsAddModalOpen(false)} className="absolute top-10 right-10 p-4 rounded-3xl bg-white/10 text-white hover:bg-rose-500 transition-all"><X size={18}/></button>
              ...
              <div className="space-y-6">
                 <div className="flex items-center gap-4 px-6 py-4 rounded-3xl bg-white/10 border border-white/5 focus-within:border-blue-500/50 transition-all">
                 ...
                 <div className="space-y-3">
                    {personnelSearch.length > 2 ? (
                      personnelPool.length > 0 ? personnelPool.map(u => (
                        <div key={u.id} className="p-5 rounded-3xl bg-white/10 border border-white/5 flex items-center justify-between group hover:bg-white/10 transition-all">
      ...
      {isEditModalOpen && editingMember && (
         <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 backdrop-blur-3xl bg-black/80">
            <div className="relative max-w-lg w-full rounded-[4rem] border border-white/10 glass-elevated p-12 space-y-8">
                <button onClick={() => { setIsEditModalOpen(false); setEditingMember(null); }} className="absolute top-10 right-10 p-4 rounded-3xl bg-white/10 text-white hover:bg-rose-500 transition-all"><X size={18}/></button>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-500">
          ...
          <div className="relative max-w-3xl w-full rounded-[4rem] border border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-hidden animate-in zoom-in-95 duration-500 glass-elevated">
            ...
            <button 
                ...
                className="absolute top-10 right-10 p-4 rounded-3xl bg-white/10 text-white hover:bg-rose-500 transition-all z-20 group"
            >
            ...
            <div className="px-12 pt-28 pb-14 space-y-12">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            {(selectedMember.skills || []).length > 0 ? selectedMember.skills?.map(s => (
                                <span key={s} className="px-4 py-1.5 bg-white/10 rounded-xl border border-white/5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{s}</span>
                ...
                {/* Footer Logistics */}
                <div className="p-8 rounded-[2.5rem] bg-white/10 border border-white/5 flex items-center justify-between">
