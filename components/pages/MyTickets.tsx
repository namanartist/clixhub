    ...
    <div className="p-4 md:p-10 max-w-7xl mx-auto space-y-10 min-h-screen">
      ...
      {filteredRegistrations.length === 0 ? (
        <div className={`flex flex-col items-center justify-center p-20 border-4 border-dashed rounded-[3rem] text-center space-y-6 ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
          <div className={`w-24 h-24 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-slate-900text-slate-700' : 'bg-slate-100 text-slate-300'}`}>
            <Ticket size={48} />
          </div>
          <div>
      ...
      {isPreviewModalOpen && selectedReg && (
          <div className="fixed inset-0 z-[2000] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-8 overflow-y-auto">
              <div className="relative w-full max-w-4xl flex flex-col items-center my-auto animate-in zoom-in-95 duration-500 py-10">
                  <div className="w-full bg-whitetext-black p-6 md:p-12 rounded-[2rem] md:rounded-[3.5rem] border-4 border-black relative overflow-hidden flex flex-col gap-8 md:gap-12 shadow-4xl">
                  ...
                  <div id="event-pass-print-area" className="w-full shadow-[0_0_150px_rgba(37,99,235,0.25)] rounded-[3rem] overflow-hidden bg-whitetext-black p-12 flex flex-col gap-10 transition-all transform scale-[1.0] md:scale-[1.05] mt-4 select-none">
                      {(() => {
      ...
      <div id="print-ticket-area" className="fixed inset-0 z-[-1] opacity-0 pointer-events-none overflow-hidden">
          {selectedReg && (
              <div className="w-[1000px] bg-whitetext-black p-12 flex flex-col gap-10">
                  {(() => {
                        const event = events.find(e => e.id === selectedReg?.eventId);
                        const club = clubs.find(c => c.id === event?.clubId);
                        const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${selectedReg?.ticketId || selectedReg?.id}`;
                        return (
                            <div className="border-[12px] border-black p-12 rounded-[4rem] relative overflow-hidden bg-whitemin-h-[600px] flex flex-col justify-between">
