  ...
  return (
    <div className={`relative h-screen w-full flex overflow-hidden transition-colors duration-700 ${isDarkMode ? 'bg-[#02040a]' : 'bg-[#F4F7FE]'}`}>
      ...
      <button
        ...
        className={`fixed top-8 right-8 z-50 p-4 rounded-2xl backdrop-blur-xl border transition-all duration-300 ${isDarkMode ? 'bg-white/10 border-white/10 text-white hover:bg-white/10' : 'bg-white/10 border-slate-200 text-slate-600 hover:shadow-xl'}`}
      >
      ...
      <div className="relative z-10 w-full flex flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-6 md:p-12 relative">
          <div className="w-full max-w-md space-y-6 animate-in fade-in zoom-in-95 duration-700">
            ...
            {/* Mode Switcher Tabs */}
            <div className={`flex rounded-2xl p-1 ${isDarkMode ? 'bg-white/10' : 'bg-slate-100'}`}>
              <button
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`group relative flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 hover:-translate-y-0.5 ${
                        isDarkMode 
                        ? 'bg-white/10 border-white/10 hover:bg-white/10 hover:border-blue-500/50' 
                        : 'bg-white/40 border-slate-200 hover:border-blue-500/50 hover:shadow-xl shadow-blue-500/10'
                      }`}
                    >
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${isDarkMode ? 'bg-white/10' : 'bg-slate-50group-hover:bg-blue-50'}`}>
                        {user.icon}
                      </div>
            ...
            {authMode === 'supabase' && (
              <div className="space-y-4">
                ...
                {/* Login / Signup Toggle */}
                <div className={`flex rounded-xl p-1 ${isDarkMode ? 'bg-white/10' : 'bg-slate-100'}`}>
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                      formType === 'login' ? (isDarkMode ? 'bg-white/10 text-white' : 'bg-whitetext-[#1B2559] shadow-sm') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')
                    }`}
                  >
                    <LogIn size={13} /> Login
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                      formType === 'signup' ? (isDarkMode ? 'bg-white/10 text-white' : 'bg-whitetext-[#1B2559] shadow-sm') : (isDarkMode ? 'text-slate-400' : 'text-slate-500')
                    }`}
                  >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-11 pr-4 py-4 rounded-2xl border text-sm font-medium transition-all outline-none focus:ring-2 focus:ring-[#3ECF8E]/50 ${
                        isDarkMode
                          ? 'bg-white/10 border-white/10 text-white placeholder:text-slate-600 focus:border-[#3ECF8E]/50'
                          : 'bg-white/40 border-slate-200 text-[#1B2559] placeholder:text-slate-400 focus:border-[#3ECF8E]'
                      }`}
                    />
                  </div>
