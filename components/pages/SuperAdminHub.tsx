    ...
    <div className="min-h-screen p-5 md:p-8 lg:p-12 space-y-8 md:space-y-12 bg-[#050505] text-[var(--text-main)] overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 reveal" style={{ animationDelay: '0.1s' }}>
          {[
              ...
              <div key={i} className="bento-card p-6 md:p-8 flex flex-col gap-3 md:gap-4 group hover:border-white/20 transition-all">
                  <div className={`p-2.5 md:p-3 w-fit rounded-[1rem] md:rounded-2xl bg-white/ ${stat.color} group-hover:scale-110 transition-transform`}>
                     <stat.icon size={20} />
                  </div>
                  <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
         ...
         <div className="lg:col-span-4 space-y-6 md:space-y-8 reveal" style={{ animationDelay: '0.2s' }}>
            <div className="bento-card p-6 md:p-10 space-y-6 md:space-y-8 relative overflow-hidden bg-white/ border-white/10 group">
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto no-scrollbar">
                    {proposals.filter(p => p.status === 'Pending').map(prop => (
                        <div key={prop.id} className="p-5 bg-white/ border border-white/5 rounded-[1.5rem] space-y-4 hover:border-amber-500/30 transition-all">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button className="px-3 h-9 bg-white/ border border-white/10 text-white/40 rounded-xl hover:text-rose-500 transition-all"><X size={14}/></button>
                            </div>
                        </div>
                    ))}
            ...
            <button onClick={() => setIsFacultyModalOpen(true)}
                    className="w-full bento-card p-6 md:p-8 flex items-center justify-between group hover:border-emerald-500/30 transition-all bg-white/ border-white/10">
         ...
         <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 reveal" style={{ animationDelay: '0.3s' }}>
            {clubs.map((club, i) => {
               ...
               return (
                  <div key={club.id} className="bento-card p-6 md:p-10 flex flex-col gap-6 md:gap-10 group relative overflow-hidden bg-white/ border-white/5">
                     ...
                     <div className="grid grid-cols-1 gap-3 md:gap-4">
                        <div className="flex items-center gap-4 md:gap-5 p-3 md:p-4 bg-white/ border border-white/5 rounded-xl md:rounded-2xl group-hover:border-white/10 transition-all">
                           ...
                           <button onClick={() => { setSelectedClub(club); setIsAppointModalOpen(true); }}
                                   className="p-2 md:p-3 bg-white/ hover:bg-primary hover:text-white rounded-lg md:rounded-xl transition-all">
                              <UserPlus size={14}/>
                           </button>
                        </div>
                        <div className="flex items-center gap-4 md:gap-5 p-3 md:p-4 bg-white/ border border-white/5 rounded-xl md:rounded-2xl group-hover:border-white/10 transition-all">
                           ...
                           <button onClick={() => { setSelectedClub(club); setIsFacultyAssignModalOpen(true); }}
                                   className="p-2 md:p-3 bg-white/ hover:bg-emerald-500 hover:text-white rounded-lg md:rounded-xl transition-all">
                              <Briefcase size={14}/>
                           </button>
                        </div>
                     </div>
          ...
          <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-8 overflow-y-auto">
              <div className="relative w-full max-w-2xl bg-[#050505] border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 space-y-6 md:space-y-10 my-auto animate-in zoom-in-95 duration-300">
                  <div className="flex justify-between items-center">
                     ...
                     <button onClick={() => setIsAppointModalOpen(false)} className="p-3 md:p-4 bg-white/ rounded-xl md:rounded-2xl text-white hover:bg-rose-500 transition-all"><X size={20}/></button>
                  </div>
                  <div className="relative">
                      ...
                      <input placeholder="Search Ledger Index..." value={appointSearch} onChange={e => setAppointSearch(e.target.value)}
                             className="w-full h-16 md:h-20 bg-white/ border border-white/10 rounded-2xl md:rounded-3xl pl-16 pr-8 font-black text-sm md:text-xl outline-none focus:border-primary/50 transition-all text-white" />
                  </div>
                  <div className="max-h-[400px] overflow-y-auto space-y-4 custom-scrollbar pr-4">
                      {filteredStudents.map(student => (
                          <div key={student.id} className="p-6 bg-white/ border border-white/5 rounded-3xl flex items-center justify-between group hover:border-primary/50 transition-all">
         ...
         <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-8 overflow-y-auto">
            <div className="relative w-full max-w-5xl bg-[#050505] border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 space-y-8 md:space-y-12 my-auto animate-in zoom-in-95 duration-300 shadow-[0_0_100px_rgba(16,185,129,0.1)]">
               <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setIsFacultyModalOpen(false)} className="p-3 md:p-4 bg-white/ rounded-xl md:rounded-2xl text-white hover:bg-rose-500 transition-all"><X size={20}/></button>
               </div>
                     ...
                     <form onSubmit={handleCreateFaculty} className="space-y-8">
                        <div className="space-y-2">
                           ...
                           <input required placeholder="Dr. Identity" value={newFaculty.name} onChange={e => setNewFaculty({...newFaculty, name: e.target.value})}
                                  className="w-full h-18 bg-white/ border border-white/10 rounded-2xl px-6 font-bold outline-none focus:border-emerald-500/50" />
                        </div>
                        <div className="space-y-2">
                  ...
                  <div className="space-y-10">
                     ...
                     <div className="max-h-[400px] overflow-y-auto space-y-4 pr-4 custom-scrollbar">
                        {facultyMembers.map(fac => (
                           <div key={fac.id} className="p-6 bg-white/ border border-white/5 rounded-3xl flex items-center justify-between">
          ...
          <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-8 overflow-y-auto">
              <div className="relative w-full max-w-2xl bg-[#050505] border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 space-y-6 md:space-y-12 my-auto animate-in zoom-in-95 duration-300">
                  <div className="flex justify-between items-center">
                     ...
                     <button onClick={() => setIsModalOpen(false)} className="p-3 md:p-4 bg-white/ rounded-xl md:rounded-2xl text-white hover:bg-rose-500 transition-all"><X size={20}/></button>
                  </div>
                  <form onSubmit={handleCreateClub} className="space-y-10">
                      <div className="space-y-2">
                         ...
                         <input required placeholder="Project Name" value={newClubData.name} onChange={e => setNewClubData({...newClubData, name: e.target.value})}
                                className="w-full h-20 bg-white/ border border-white/10 rounded-3xl px-8 font-black text-xl outline-none focus:border-primary/50" />
                      </div>
                      <div className="grid grid-cols-2 gap-10">
                         <div className="space-y-2">
                            ...
                            <select value={newClubData.category} onChange={e => setNewClubData({...newClubData, category: e.target.value as any})}
                                    className="w-full h-18 bg-white/ border border-white/10 rounded-2xl px-6 font-black outline-none appearance-none">
                         ...
                         <div className="space-y-2">
                            ...
                            <input type="color" value={newClubData.themeColor} onChange={e => setNewClubData({...newClubData, themeColor: e.target.value})}
                                   className="w-full h-18 bg-white/ border border-white/10 rounded-2xl px-6 cursor-pointer" />
                         </div>
                      </div>
          ...
          <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 md:p-8 overflow-y-auto">
              <div className="relative w-full max-w-2xl bg-[#050505] border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 space-y-6 md:space-y-10 my-auto animate-in zoom-in-95 duration-300">
                  <div className="flex justify-between items-center">
                     ...
                     <button onClick={() => setIsFacultyAssignModalOpen(false)} className="p-3 md:p-4 bg-white/ rounded-xl md:rounded-2xl text-white hover:bg-rose-500 transition-all"><X size={20}/></button>
                  </div>
                  <div className="relative">
                      ...
                      <input placeholder="Search Personnel Registry..." value={facultyAssignSearch} onChange={e => setFacultyAssignSearch(e.target.value)}
                             className="w-full h-20 bg-white/ border border-white/10 rounded-3xl pl-16 pr-8 font-black text-xl outline-none focus:border-emerald-500/50 transition-all" />
                  </div>
                  <div className="max-h-[400px] overflow-y-auto space-y-4 custom-scrollbar pr-4">
                      {filteredFacultyForAssign.map(fac => (
                          <div key={fac.id} className="p-6 bg-white/ border border-white/5 rounded-3xl flex items-center justify-between group hover:border-emerald-500/50 transition-all">
                              <div className="flex items-center gap-6">
